...
(not currently being used in project) tellus_PM10_sensor_frame.py = was my first attempt to find all of the locations for sensors and map them. this created the masterframe.csv

(not currently being used in project) plotting.py = was my first attempt to map out all unique sensors. uses the masterframe.csv

six_maps.py = this is the maps of the 6 seasons of data I have using Clean_PlotData.csv. I also plan on using contourf to make those maps look a lot more like a heat map.
...
(not currently being used in project) finding_sensors.py = this was an attempt to reverse geo to find all of the sensors in their uniqe city.
